...
public record PayementResponseBase(int Id,string internalref, DateTime Date, decimal Amount, decimal fees, string MeterSerial, int? MeterId, string Agent);
public record BuyCreditResponse(int Id, int Amount,decimal fees, string SerialNumber, string Reference, DateTime date, string InternalReference, string Code, double Credit, string Seller)
{
    public string Message() => $"Vous avez payé {Amount} FCFA de crédit CASH POWER (Ref: {InternalReference}) ce {date.ToLongDateString()}. Le Code de Votre paiement est : {Code} (KWh: {Credit}), Compteur: {SerialNumber} Frais: {fees}";
}
public static partial class DataConverter
{
    ...
    public async static Task<PayementResponseBase> GetPayementResponse(this Payment payment, IUserService userService)
    {
        ...
        return new PayementResponseBase(payment.Id,payment.InternalReference, payment.CreatedOn, payment.Amount,payment.BilledAmount, payment.SerialNumber,null, user.Data.UserFullName);
    }
    ...
    {
        var user = await userService.GetAsync(payment.CreatedBy);
        return new PayementResponseBase(payment.Id, payment.InternalReference, payment.CreatedOn, payment.Amount,payment.BilledAmount, payment.SerialNumber, meterId, user.Data.UserFullName);
    }
